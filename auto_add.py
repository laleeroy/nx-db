...
def get_game_info_from_titles_db(title_id, titles_file="titles.json"):
    ...
    try:
        ...
        print(f"[ERROR] Could not read {titles_file}: {e}")
    return None
...
def fetch_game_info(title_id):
    ...
    if game_data and is_data_complete(game_data):
        print(f"[OK] Found complete data in titles.json")
        return game_data
    else:
        print(f"[INFO] Data incomplete or not found in titles.json, checking tinfoil.media...")
